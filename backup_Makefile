...
approve-engine-to-spend-collateral-tokens:
	cast send $(COLLATERAL_TOKEN_ADDRESS) "approve(address,uint256)" $(STABLECOIN_ENGINE_ADDRESS) 499000 --private-key $(DEPLOYER_PRIVATE_KEY)

depositTokensAndMintStablecoin:
	cast send $(STABLECOIN_ENGINE_ADDRESS) "depositCollateralAndMintStablecoin(address,uint256,uint256)" $(COLLATERAL_TOKEN_ADDRESS) 99000 33000 --private-key $(DEPLOYER_PRIVATE_KEY)
